...
class AppUpdateInstallerImpl : AppUpdateInstaller {
    private val context get() = AndroidAppContext.instance
     ...
     * this app's targetSdk - that throws `FileUriExposedException` - so `androidApp`'s manifest
     * declares a `FileProvider` whose authority matches [FILE_PROVIDER_AUTHORITY_SUFFIX] below,
     * scoped to exactly the cache subdirectory the update APK is downloaded into (see
    ...
    private companion object {
        // Must exactly match the FileProvider <provider> authority declared in androidApp's
